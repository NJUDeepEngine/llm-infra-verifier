...
# ── SPMD Type Manipulation ops (from meta-pytorch/spmd_types) ────────────────
...
class Reinterpret(IROp):
    ...
    def vjp(self, ctx, grad_output):
        # Backward: reverse the type transition
        # R→V forward: backward is V→R (needs all_gather)
        # R→P forward: backward is P→R (needs all_reduce)
